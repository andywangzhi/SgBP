$(function(){
			$('#list').datagrid({
				title:'<span style="color:#4682B4">字典列表</span>',
				iconCls:'icon-edit',
				width:'auto',
				height:390,
				nowrap: false,
				striped: true,
				collapsible:true,
				url:path+'/system/conf!findAll.action?datestr='+new Date().getTime(),
				...
				columns:[[
					...
					{field:'parentName',title:'父名称',width:100},
					...
					{field:'remark',title:'备注',width:285}
				]],
				pagination:true,
				rownumbers:true,
				toolbar:[{
	...
	function loadType(){
	  $.ajax({
	     url:path+"/system/conf!findTypeForJson.action?datestr="+new Date().getTime(),
	     dataType:"json",
	...
	function addRecord(){
	    openWindow(path+"/system/conf!viewAdd.action?datestr="+new Date().getTime(),570,550);
	}
	...
	function editRecord(){
		...
		if(rows==null || rows.length == 0){
		  ...
		  if(rows[0].valid != '1'){
		    ...
		    openWindow(path+"/system/conf!viewEdit.action?id="+rows[0].id+"&datestr="+new Date().getTime(),570,350);
		  }
		}
	}
		   ...
		   alert('请选择记录');
             ...
             $.messager.confirm('系统提示', '您确定要删除选择的记录吗?', function(r) {
                 if (r) {
			           ...
			           $.get(path+"/system/conf!saveStatus.action?status=2&ids=" + ids.join(':')+"&datestr="+new Date().getTime(), function(data, status, Request){
			              $(".showMsg").html(data);
			               location.reload();
			           })
                 }
		   ...
		   alert('不能激活已删除的记录');
             ...
             $.messager.confirm('系统提示', '您确定要激活选择的记录吗?', function(r) {
                 if (r) {
			           ...
			           $.get(path+"/system/conf!saveStatus.action?status=1&ids=" + ids.join(':')+"&datestr="+new Date().getTime(), function(data, status, Request){
			              $(".showMsg").html(data);
			               location.reload();
			           })
                 }
		...
		if(ids==null || ids.length == 0){
		   ...
		   alert('不能停用已删除的记录');
             ...
             $.messager.confirm('系统提示', '您确定要停用选择的记录吗?', function(r) {
                 if (r) {
			           $.blockUI({ message: '<div class="showMsg">正在处理中,请稍等....</div>' });
			           $.get(path+"/system/conf!saveStatus.action?status=0&ids=" + ids.join(':')+"&datestr="+new Date().getTime(), function(data, status, Request){
			              $(".showMsg").html(data);
			               location.reload();
			           })
                 }
	...
	function viewRecord(){
		...
		if(rows==null || rows.length == 0){
		  ...
		  alert('只能选择一条记录');
	       ...
	       openWindow(path+"/system/conf!view.action?id="+rows[0].id+"&datestr="+new Date().getTime(),570,350);
		}
	}