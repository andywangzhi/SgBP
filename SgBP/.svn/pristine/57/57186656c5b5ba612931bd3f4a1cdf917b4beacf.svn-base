...
<link rel="stylesheet" type="text/css" href="<%=path%>/business/css/reset.css" />
		...
		function prompt(){ 
			jQuery.messager.promptforpassword('提示:','请输入密码.',function(r){ 
				$("#frm").attr("action","<%=path%>/soa/rctj!queryPcountyforReport.action?password="+r+"&menuId=<%=request.getParameter("menuId") + _strType%>");
	    		$("#frm").submit();
			}) 
		} 
    	...
    	$(function(){
	    	...
	    	function find(){
					...
					$("#frm").attr("action","<%=path%>/soa/rctj!queryPcountyforReport.action?menuId=<%=request.getParameter("menuId") + _strType%>");
	    			$("#frm").submit();
		    	}
...
<div class="web_paht">
    <div class="row_l web_paht_l">您的位置：人才统计监控&nbsp;&gt;&nbsp;韶关市人才上报统计&nbsp;&gt;&nbsp;韶关市人才上报统计
    </div>
    <div class="row_r web_paht_r tool">
          <a class="refresh" href="<%=path%>/soa/rctj!queryPcountyforReport.action?menuId=<%=request.getParameter("menuId") + _strType%>">刷新</a>
...
<div class="main_in">
         ...
         <form action="<%=path%>/soa/rctj!queryPcountyforReport.action?menuId=<%=request.getParameter("menuId") + _strType%>" method="post" name="frm" id="frm">
              <div class="table_edit">
              <input id="isCanSee" name="isCanSee" type="hidden" value='<s:property value="isCanSee"/>'/>
              <input id="isreport" name="form.retrieveSO.isreport" type="hidden" value='true'/>
