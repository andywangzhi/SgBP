...
<head>
	...
	<script type="text/javascript"> 
		...
		function getSearch(){
			...
			if(flag){
				document.getElementById('frm').action = path+'/soa/RsPerson!dbQuery.action?menuId=${menuId}';
		...
		function audOk() {
			...
			if(selectInt == 0) {
				...
				document.getElementById('frm').action = '<%=path%>/soa/RsPerson!uDbState.action?menuId=${menuId}';
				document.getElementById('frm').submit();
			}
		}
...
<body>
<form id="frm" name="frm" action="<%=path%>/soa/RsPerson!uDbState.action?menuId=${menuId}" method="post" >  

<div class="web_paht">
    <div class="row_l web_paht_l">您的位置：个人信息审核&nbsp;&gt;&nbsp;手工比对</div>
</div>
<div class="main_in">
		...
		<div class="table_edit">
                  <table>
                  	...
                  	<tr>
                  		...
                  		<th nowrap="nowrap" style="padding-right: 0px;">手工比对状态：</th>
                  		<td>
                  			<select id="auditstate" name="form.queryListSO.auditstate" style="width: 150px;">
                  				<option value="">请选择</option>
                  				<s:if test="form.queryListSO.auditstate == \"000\"">
                  				<option value="000" selected="selected">未比对</option>
                  				</s:if>
                  				<s:else><option value="000">未比对</option></s:else>
                  				<s:if test="form.queryListSO.auditstate == \"100\"">
                  				<option value="100" selected="selected">已比对已通过</option>
                  				</s:if>
                  				<s:else><option value="100">已比对已通过</option></s:else>
                  				<s:if test="form.queryListSO.auditstate == \"200\"">
                  				<option value="200" selected="selected">已比对未通过</option>
                  				</s:if>
                  				<s:else><option value="200">已比对未通过</option></s:else>
                  			</select>
                  		</td>
                  	</tr>
                  	<tr>
                  		<th nowrap="nowrap" style="padding-right: 0px;">比对日期-开始日期：</th>
                  		...
                  		<th nowrap="nowrap" style="padding-right: 0px;">比对日期-结束日期：</th>
                  ...
                  <table>
                      ...
                      <tbody class="bg_x">
                      	<s:if test="form.dtoList.size() > 0 ">
				            <s:iterator value="form.dtoList" id="personInfo" status="st">
								  ...
								  <td nowrap="nowrap">
								  	...
								  	<input type="checkbox" name="form.dtoList[<s:property value="#st.index" />].auditstate" value="100" <s:if test="#personInfo.auditstate == \"100\"">disabled="disabled"</s:if>/>
				                  ...
				                  <td nowrap="nowrap">
				                  	<s:if test="#personInfo.auditstate == \"100\""><font color="green">已比对已通过</font></s:if>
				                  	<s:elseif test="#personInfo.auditstate == \"200\"">已比对未通过</s:elseif>
				                  	<s:else>
				                  		<font color="red">未比对</font>
				                  	</s:else>
				                  </td>
				          ...
				          <tr>
				          	<td colspan="15">
				          		<div class="row_l">
				          		<a class="btn_search" onclick="audOk();">比对通过</a>&nbsp;&nbsp;
				          		<a class="btn_search" onclick="audFail();">比对不通过</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
