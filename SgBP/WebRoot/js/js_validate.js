...
var checkData = new function() {   
    ...
    */  
    ...
    this.isMoreSpan = function(id, moreLen) {   
  	...
  	/*
  		验证手机号码
  		验证13系列和150-159(154除外)
  		、180、185、186、187、188、189几种号码，长度11位
  	*/
  	
  	this.isMobile = function(id){
  		var val = document.getElementById(id);
  		alert(/^13\d{9}$/g.test(val)+"1");
  		alert(/^15[0-35-9]\d{8}$/g.test(val)+"2");
  		alert(/^18[05-9]\d{8}$/g.test(val)+"3");
  		
  		  if(/^13\d{9}$/g.test(val)||(/^15[0-35-9]\d{8}$/g.test(val))||   
  	...
  	//	alert(value);
  		...
  		if(val.value.length>0){
  			 if(!tel.test(val.value)){
  			 	val.select();   
           		var str = "您输入的电话号码有错误。区号和电话号码之间请用-分割";
            	alert(str); 
            //	$('#'+id).addClass("validate");
            //	$("#"+id).blur();
  				return false;
    ...
    */  
    ...
    this.isEmptySpan = function(id) {
        var val = document.getElementById(id);
        if (this.isEmpty($.trim(val.value))) {
            var str = "* 不允许为空";   
            val.select();   
            alert(str);
            //$('#'+id).addClass("validate");
