$(function(){
			$('#list').datagrid({
				title:'<span style="color:#4682B4">数据字典</span>',
				iconCls:'icon-edit',
				width:'auto',
				height:390,
				nowrap: false,
				striped: true,
				collapsible:true,
				url:path+'/system/dictionary!findAll.action?datestr='+new Date().getTime(),
				queryParams:{'code':'','name':'','type':'','valid':'','sensitive':'','searchflag':'0'},
				sortName: 'createDate',
				sortOrder: 'desc',
				remoteSort: true,
				idField:'id',
				...
				columns:[[
					...
					{field:'remark',title:'备注',width:305}
				]],
				pagination:true,
				rownumbers:true,
				toolbar:[{
	...
	function queryRecord(){
		...
		var sensitive='';
		$("[name='search_sensitive']:checked").each(function(){
		      sensitive =$(this).val();
		});
	...
	function addRecord(){
	    openWindow(path+"/system/dictionary!viewAdd.action?datestr="+new Date().getTime(),570,350);
	}
	...
	function editRecord(){
		...
		if(rows==null || rows.length == 0){
		  ...
		  if(rows[0].valid != '1'){
		    ...
		    openWindow(path+"/system/dictionary!viewEdit.action?id="+rows[0].id+"&datestr="+new Date().getTime(),570,350);
		  }
		}
	}
		   ...
		   alert('请选择记录');
             ...
             $.messager.confirm('系统提示', '您确定要删除选择的记录吗?', function(r) {
                 if (r) {
			           ...
			           $.get(path+"/system/dictionary!saveStatus.action?status=2&ids=" + ids.join(':')+"&datestr="+new Date().getTime(), function(data, status, Request){
			              $(".showMsg").html(data);
			               location.reload();
			           })
                 }
		   ...
		   alert('不能激活已删除的记录');
             ...
             $.messager.confirm('系统提示', '您确定要激活选择的记录吗?', function(r) {
                 if (r) {
			           ...
			           $.get(path+"/system/dictionary!saveStatus.action?status=1&ids=" + ids.join(':')+"&datestr="+new Date().getTime(), function(data, status, Request){
			              $(".showMsg").html(data);
			               location.reload();
			           })
                 }
		...
		if(ids==null || ids.length == 0){
		   ...
		   alert('不能停用已删除的记录');
             ...
             $.messager.confirm('系统提示', '您确定要停用选择的记录吗?', function(r) {
                 if (r) {
			           $.blockUI({ message: '<div class="showMsg">正在处理中,请稍等....</div>' });
			           $.get(path+"/system/dictionary!saveStatus.action?status=0&ids=" + ids.join(':')+"&datestr="+new Date().getTime(), function(data, status, Request){
			              $(".showMsg").html(data);
			               location.reload();
			           })
                 }
	...
	function viewRecord(){
		...
		if(rows==null || rows.length == 0){
		  ...
		  alert('只能选择一条记录');
	       ...
	       openWindow(path+"/system/dictionary!view.action?id="+rows[0].id+"&datestr="+new Date().getTime(),570,350);
		}
	}